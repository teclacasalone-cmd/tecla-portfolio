  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[#faf8f5]/90 backdrop-blur-sm border-b border-[#e5e0d8]">
      <div className="max-w-5xl mx-auto px-6 flex items-center justify-between h-14">
        <a href="#" className="font-[var(--font-heading)] text-sm font-semibold tracking-wide">TC</a>
      ...
      {open && (
        <div className="md:hidden bg-[#faf8f5] border-b border-[#e5e0d8] px-6 py-4 flex flex-col gap-3">
          {links.map((l) => (
            <a key={l.href} href={l.href} className="text-sm text-[#888] hover:text-[#1a1a1a]" onClick={() => setOpen(false)}>
              {l.label}
            </a>
          ))}
        </div>
...
function Skills() {
  const groups = [
    ...
    { label: "Advertising", items: ["Google Ads", "Meta Ads", "LinkedIn Ads", "Funnel TOFU / MOFU / BOFU"] },
    { label: "Analytics & SEO", items: ["Google Analytics 4", "Search Console", "UX/UI Optimization", "Keyword Research"] },
    { label: "Tools", items: ["Shopify", "Canva", "GoHighLevel", "WordPress", "Eversell"] },
  ];
          ...
          {groups.map((g, gi) => (
            <ScaleIn key={g.label} delay={gi * 0.1}>
              ...
              <ul className="space-y-3">
                {g.items.map((item) => (
                  <li key={item} className="text-[#777] text-sm flex items-center gap-3">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#c44536] shrink-0" />
                    {item}
                  </li>
                ))}
              </ul>
      ...
      <div className="max-w-5xl mx-auto grid md:grid-cols-5 gap-16">
        ...
        <SlideRight className="md:col-span-3" delay={0.15}>
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button type="submit" className="px-8 py-3 bg-[#c44536] hover:bg-[#a93a2d] text-white text-sm font-medium rounded-full transition-colors">
              Invia messaggio
            </button>
  ...
  return (
    <footer className="py-8 px-6 border-t border-[#e5e0d8]">
      <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
        <p className="text-xs text-[#999]">&copy; {new Date().getFullYear()} Tecla Casalone</p>
